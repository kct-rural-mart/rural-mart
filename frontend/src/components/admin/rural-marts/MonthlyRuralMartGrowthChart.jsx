...
import { MONTHLY_MART_GROWTH_DATA } from '../../../lib/newPages/mockData'
    ...
    return (
      <div className="bg-brand-text/95 text-white border border-brand-accent p-3 rounded-xl shadow-xl text-xs space-y-1.5 min-w-[220px]">
        <div className="font-bold text-sm text-white border-b border-white/20 pb-1 flex justify-between items-center">
          ...
          <span className="text-[10px] text-white/60">Monthly Snapshot</span>
        </div>
...
export default function MonthlyRuralMartGrowthChart() {
  return (
    <div className="bg-brand-surface border border-brand-border rounded-2xl p-4 shadow-xs hover:shadow-md hover:-translate-y-0.5 hover:border-brand-primary/40 transition-all duration-200 flex flex-col justify-between">
      <div className="flex items-center justify-between pb-3 mb-2 border-b border-brand-border/60">
        <h3 className="text-sm font-bold text-brand-text tracking-tight">Monthly Rural Mart Growth</h3>
      </div>

      <div className="w-full h-72 md:h-80 relative">
        {MONTHLY_MART_GROWTH_DATA.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={MONTHLY_MART_GROWTH_DATA} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
              ...
              <YAxis yAxisId="left" domain={[0, 15]} tick={{ fill: CHART_COLORS.textMuted, fontSize: 10, fontWeight: 500 }} axisLine={false} tickLine={false} tickFormatter={(v) => `${v}`} />
              <YAxis yAxisId="right" orientation="right" domain={[0, 3]} tick={{ fill: CHART_COLORS.textMuted, fontSize: 10, fontWeight: 500 }} axisLine={false} tickLine={false} tickFormatter={(v) => `₹${v}Cr`} />
              ...
              <Line yAxisId="left" type="monotone" dataKey="activeMarts" name="Active Marts" stroke={CHART_COLORS.primary} strokeWidth={3} dot={{ r: 4, fill: CHART_COLORS.primary }} activeDot={{ r: 7 }} />
              <Line yAxisId="right" type="monotone" dataKey="networkSalesCr" name="Network Sales (₹Cr)" stroke="#7e22ce" strokeWidth={3} dot={{ r: 4, fill: '#7e22ce' }} />
              <Line yAxisId="left" type="monotone" dataKey="avgScore" name="Avg Score (scaled)" stroke={CHART_COLORS.info} strokeWidth={2} strokeDasharray="3 3" dot={{ r: 3, fill: CHART_COLORS.info }} />
            </LineChart>
          </ResponsiveContainer>
        )}
      </div>
