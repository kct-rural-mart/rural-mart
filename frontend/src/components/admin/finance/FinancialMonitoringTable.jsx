...
import { Search, ArrowUpDown, ChevronLeft, ChevronRight, Eye, TrendingUp, TrendingDown, Minus, Download, CheckCircle2, AlertCircle, Clock } from 'lucide-react'
...
export default function FinancialMonitoringTable({ financialMarts, onSelectMart, searchQuery, setSearchQuery }) {
  ...
  const exportToCsv = () => {
    ...
    const rows = sortedMarts.map((m) => [m.name, m.district, m.status, m.salesRaw, m.procurementRaw, m.netProfitRaw, m.profitMargin])
    ...
    <div className="bg-brand-surface border border-brand-border rounded-xl p-4 shadow-xs hover:shadow-md hover:-translate-y-0.5 hover:border-brand-primary/40 transition-all duration-200 flex flex-col justify-between h-full">
      <div>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 pb-3 border-b border-brand-border">
          <h3 className="text-sm font-bold text-brand-text tracking-tight">Financial Monitoring Table</h3>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={exportToCsv}
              className="p-1.5 rounded-lg border border-brand-border text-brand-text-muted hover:bg-brand-bg-subtle transition-colors"
              title="Export CSV"
            >
            ...
            <thead>
              <tr className="border-b border-brand-border text-brand-text-muted font-semibold uppercase text-[10px] tracking-wider bg-brand-bg-subtle">
                ...
                <th className="py-2.5 px-3 text-right">
                  <button onClick={() => handleSort('salesRaw')} className="flex items-center gap-1 ml-auto hover:text-brand-text">
                    Sales <ArrowUpDown className="w-3 h-3" />
                  </button>
                </th>
                <th className="py-2.5 px-3 text-right">
                  <button onClick={() => handleSort('procurementRaw')} className="flex items-center gap-1 ml-auto hover:text-brand-text">
                    Procurement <ArrowUpDown className="w-3 h-3" />
                  </button>
                </th>
                <th className="py-2.5 px-3 text-right">
                  <button onClick={() => handleSort('netProfitRaw')} className="flex items-center gap-1 ml-auto hover:text-brand-text">
                    Net Profit <ArrowUpDown className="w-3 h-3" />
                  </button>
                </th>
                <th className="py-2.5 px-3 text-right">
                  <button onClick={() => handleSort('profitMargin')} className="flex items-center gap-1 ml-auto hover:text-brand-text">
                  ...
                  <tr key={mart.id} className="hover:bg-brand-bg-subtle transition-colors group">
                    <td className="py-3 px-3">
                      <div className="flex flex-col">
                        ...
                        <span className="text-[10px] text-brand-text-subtle font-medium">{mart.district} District</span>
                      </div>
                    </td>

                    <td className="py-3 px-3 text-right font-bold text-brand-text">{mart.salesDisplay}</td>
                    <td className="py-3 px-3 text-right text-brand-text-muted">{mart.procurementDisplay}</td>
                    <td className="py-3 px-3 text-right font-bold text-brand-primary">{mart.netProfitDisplay}</td>

                    <td className="py-3 px-3 text-right">
                      <span
                        className={`font-bold px-1.5 py-0.5 rounded text-xs ${
                          mart.profitMargin >= 14 ? 'bg-brand-primary-light text-brand-primary-dark' : mart.profitMargin > 0 ? 'bg-brand-warning-light text-brand-warning-dark' : 'bg-brand-danger-light text-brand-danger'
                        }`}
                      >
                    ...
                    <td className="py-3 px-3 text-center">
                      <div className="flex items-center justify-center">
                        {mart.trend === 'up' && (
                          <span className="inline-flex items-center gap-0.5 text-brand-primary font-bold text-[10px] bg-brand-primary-light px-1.5 py-0.5 rounded">
                        ...
                        {mart.trend === 'flat' && (
                          <span className="inline-flex items-center gap-0.5 text-brand-text-subtle font-bold text-[10px] bg-brand-bg-subtle px-1.5 py-0.5 rounded">
                    ...
                    <td className="py-3 px-3 text-center">
                      <button
                        ...
                        className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-brand-primary-light text-brand-primary-dark font-bold hover:bg-brand-primary hover:text-white transition-all text-xs"
      ...
      <div className="flex items-center justify-between pt-3 mt-3 border-t border-brand-border text-xs text-brand-text-muted">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className="p-1.5 rounded-lg border border-brand-border disabled:opacity-40 hover:bg-brand-bg-subtle transition-colors"
          >
