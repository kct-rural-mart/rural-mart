    ...
    return (
      <div className="bg-brand-text text-white border border-brand-accent p-3 rounded-xl shadow-xl text-xs space-y-2 min-w-[230px] pointer-events-none select-none">
        ...
        <div className="flex items-center justify-between text-brand-warning-light font-bold pt-1 border-t border-white/20">
          ...
          <span>{data.repeatPurchaseRate}%</span>
        </div>
      </div>
    )
  }
...
export default function FarmerGrowthAndRetentionChart({ trendData = [] }) {
  return (
    <div className="bg-brand-surface border border-brand-border rounded-xl p-4 shadow-xs hover:shadow-md hover:-translate-y-0.5 hover:border-brand-primary/40 transition-all duration-200 flex flex-col justify-between">
      <div className="flex items-center justify-between pb-3 mb-2 border-b border-brand-border">
        <h3 className="text-sm font-bold text-brand-text tracking-tight">Farmer Growth &amp; Repeat Purchase Trend</h3>
      </div>
        ...
        {trendData.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={trendData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
              ...
              <YAxis yAxisId="left" tick={{ fill: CHART_COLORS.textMuted, fontSize: 11, fontWeight: 500 }} axisLine={false} tickLine={false} />
              <YAxis yAxisId="right" orientation="right" domain={[0, 100]} tick={{ fill: CHART_COLORS.warning, fontSize: 11, fontWeight: 600 }} axisLine={false} tickLine={false} tickFormatter={(v) => `${v}%`} />
              ...
              <Line yAxisId="left" type="monotone" dataKey="newFarmers" name="New Farmers" stroke={CHART_COLORS.accent} strokeWidth={2} dot={{ r: 3, fill: CHART_COLORS.accent }} isAnimationActive={false} />
              <Line yAxisId="right" type="monotone" dataKey="repeatPurchaseRate" name="Repeat Purchase Rate %" stroke={CHART_COLORS.warning} strokeWidth={2.5} dot={{ r: 4, fill: CHART_COLORS.warning }} activeDot={{ r: 6 }} isAnimationActive={false} />
            </LineChart>
          </ResponsiveContainer>
        )}
      </div>
