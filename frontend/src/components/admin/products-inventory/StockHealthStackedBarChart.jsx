...
import { STOCK_HEALTH_BY_MART } from '../../../lib/newPages/mockData'
  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-brand-text text-white border border-brand-accent p-3 rounded-xl shadow-xl text-xs space-y-1.5 min-w-[210px]">
        ...
        <div className="flex items-center justify-between text-brand-warning-light font-bold">
          <span className="flex items-center gap-1.5">
...
export default function StockHealthStackedBarChart() {
  return (
    <div className="bg-brand-surface border border-brand-border rounded-2xl p-4 shadow-xs hover:shadow-md hover:-translate-y-0.5 hover:border-brand-primary/40 transition-all duration-200 flex flex-col justify-between">
      <div className="flex items-center justify-between pb-3 mb-2 border-b border-brand-border/60">
        <h3 className="text-sm font-bold text-brand-text tracking-tight">Stock Health Breakdown</h3>
      </div>

      <div className="w-full h-72 md:h-80 relative">
        {STOCK_HEALTH_BY_MART.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={STOCK_HEALTH_BY_MART} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
              ...
              <Bar dataKey="lowStock" name="Low Stock" stackId="stock" fill={CHART_COLORS.warning} radius={[0, 0, 0, 0]} barSize={18} />
